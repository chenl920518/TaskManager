...
<body>
	<div class="quality quality_report">
		...
		<div class="wrap">
			<div class="wrap_top">
				...
				<div class="imgs">
					<ul  class="clearfix" >
						...
						<c:forEach var="picUrl" items="${qualityReport.picUrls}">
						    <li>
			...
			<div class="a0 div_a"><a href="javaScript:void(0);" onclick="download_file(${qualityReport.id})">压缩包下载链接</a></div>
			<div class="a0">
				<span>本报告是：</span>
...
<script>
      ...
      window.location.href ="http://192.168.1.82:8196/TaskManager/jsp/quality_report_m.jsp"; // 如果是苹果机，就打开苹果的页面

	  } else if (/(Android)/i.test(navigator.userAgent)) {//如果是安卓机，就打开安卓页面
	
	      window.location.href ="http://192.168.1.82:8196/TaskManager/jsp/quality_report_m.jsp";
	
	  } else {
	
	      window.location.href ="http://192.168.1.82:8196/TaskManager/jsp/quality_report_pc.jsp";
	  };
			...
			yes:function(){
				...
				$.ajax({
				     ...
				     success:function(data){
				    	 var json = eval("(" + data +")");
						 if(json.ok){
							  window.location.href="${ctx}/quality/viewQuality?id="+id;
						  }else{
